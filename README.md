...
## Contributing
PRs welcome! Each DataDAO may ship its own GUI around the same file protocol. For experimental clients, see [haven-clients-experimental](https://github.com/Haven-hvn/haven-clients-experimental#contributing) (`clients/<name>/` — SLOP testbed, merged PRs count under `haven`); for awesome-list curation, PR to this repo (`awesome.md` / `interfaces/`).
